...
- Run the application with `python main.py` or the installed `haiku-forge` entry point only when interactive output is needed.

## Managed artifacts and data

- Generated WAV files and temporary output remain ignored.
- Treat committed reference text under `outputs/` as behavior evidence; update it only through an intentional, reproducible generation run.

## Integration and claim boundaries

- Audio clips, datasets, and coursework artifacts may have separate ownership and usage terms.
- The canonical project name is HaikuForge AI, and the repository grants no repository-wide software license.
- **Verified local capability:** Python 3.11/3.12, lint, compile, and security checks.
- **Known boundary:** Canonical project name is HaikuForge AI; no repository-wide license.
